...
export class SenseContent {
  ...
  constructor(
    ...
    this.partOfSpeech = dictionarySettings["partOfSpeech"].searchOptions.find(
      (it) => it.id === json["part_of_speech"]
    ).name;
    this.flag = dictionarySettings["lexicon"].searchOptions.find(
      (it) => it.id === json["lexicon"]
    ).name;
    this.domain = dictionarySettings["domain"].searchOptions.find(
      (it) => it.id === json["domain"]
    ).name;
  ...
  private setContent(jsonData, dictionarySettings) {
    ...
    if (this.flag === "Germanet") {
      fieldNames = {
        ...
        part_of_speech: {
          viewName: "Part of speech",
          type: "simple_dict",
          dictName: "partOfSpeech",
        },
      };
    } else {
      fieldNames = {
          ...
          values: [
            {
              name: dictionarySettings[
                fieldNames[key].dictName
              ].searchOptions.find((it) => it.id === jsonData[key]).name,
              searchQuery: null,
            },
          ],
        };
